layui.use(['form', 'table', 'miniPage', 'element', 'myAjax','laydate'], function () {
    ...
    let url = Requesthttp + 'activity/announcementList';
    table.render({
        elem: '#currentTableId',
        url: url,
        ...
        cols: [[
            ...
            {field: 'title', minWidth: 200, title: Subject_Name+'名称', align: "center"},
            {
                field: 'type', width: 100, title: Subject_Name+'类型', align: "center",
                templet: function (d) {
                    if(d.type == 1){
                        return '图片';
                    }else if(d.type==2){
            ...
            {field: 'content', width: 200, title: Subject_Name+'内容', align: "center"},
            {
    ...
    table.on('toolbar(currentTableFilter)', function (obj) {
        if (obj.event === 'add') {   // 监听添加操作
            var content = miniPage.getHrefContent('page/activity/ann/add.html');
            ...
            layer.confirm('确定导出公告数据？', function (index) {
                const url = Requesthttp + 'activity/exportAnnList' + '?' + requestData;
                downloadUrl(url);
                layer.close(index);
            });
        }
    ...
    table.on('tool(currentTableFilter)', function (obj) {
        ...
        if (obj.event === 'edit') {
            ...
            let edit_data = [];
            myAjax.authAjax("activity/annGetInfo", "get",
            ...
            myAjax.authAjax("activity/annChangeState", "post",
            ...
            layer.confirm('你确认要删除该行数据吗？', function (index) {
                myAjax.authAjax('activity/annDel', 'post',
